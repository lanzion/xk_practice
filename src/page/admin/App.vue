        ...
        <div class="main-box">
            ...
            <el-menu
                ...
                <template v-for="item in nav">
                    <el-menu-item class="is-level-1" v-if="item.url" :index="item.url" :key="item.id">
                    ...
                    <el-submenu v-else :index="item.id" :key="item.id">
                        <div class="is-level-1" slot="title">
                            <span class="icon-icon_tongji-fenxi" style="fontSize:18px;" v-if="item.menuId=='tongji-fenxi'"><span class="path1"></span><span class="path2"></span><span class="path3"></span><span class="path4"></span><span class="path5"></span><span class="path6"></span></span>
                           
                              <i :class="['el-icon-tickets', `icon-icon_${item.menuId}`]" v-else></i>
                            <span>{{ item.name}}</span>
                        </div>
                        <el-menu-item v-for="child in item.children" :index="child.url" :class="{ 'is-active': breadcrumb.some(x => x.url === child.url) }" :key="child.id">
                            <i class="el-icon-"></i>
            ...
            <div class="content-box">
                <el-breadcrumb v-show="breadcrumb.length" class="breadcrumb"  separator-class="el-icon-arrow-right">
                    <el-breadcrumb-item v-for="item in breadcrumb" :to="{ path: item.url }" :key="item.id">{{ item.name }}</el-breadcrumb-item>
                </el-breadcrumb>
                <router-view />
            </div>
        </div>
...
//   },
    ...
    computed: {
        ...mapState({
            ...
            // 全部菜单权限
            nav: state => ((state.login.permission || [])[0] || {}).children || []
        }),
        permission() {
    ...
    components: {
        'user-avatar': resolve => require(['@/components/common/admin-head-img'], resolve)
    },
    created() {
    ...
    watch: {
        identity: {
            handler: function (identity) {
                if (identity !== 'admin' || identity !== 'education' || identity !== 'school') {
