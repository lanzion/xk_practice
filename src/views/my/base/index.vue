  ...
  <div class="container">
    ...
    <div class="g-search--wrap clearfix">
      ...
      <el-form :inline="true" class="fr">
        <el-form-item>
          <el-input v-model="name" placeholder="请输入基地/机构关键字" @keyup.native.enter="resetPage"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <div class="base">
      <ul class="base-ul">
        <li class="base-li" v-for="(g,index) in datas" :key="index">
          <div class="base-li-img fl" @click="go(g.id,g.courseNum)">
            <ov-image :type="1" :src-data="getFileUrl(g.cover)" :img-height="'222px'"></ov-image>
          </div>
          <div class="base-li-conent fl">
            <h3 @click="go(g.id,g.courseNum)">{{cutString(g.name,140)}}</h3>
          ...
          setTimeout(() => {
            if (node.level == 0) {
              requestRegion({ pcode: 440000 })
                .then(res => {
                  const cities = res.data.appendInfo.list.map((value, i) => ({
                    value: value.code,
                    label: value.name,
                    leaf: node.level >= 1
                  }));
            ...
            if (node.level == 1) {
              requestRegion({ pcode: node.value })
                .then(res => {
                  const areas = res.data.appendInfo.list.map((value, i) => ({
                    value: value.code,
                    label: value.name,
                    leaf: node.level >= 1
                  }));
