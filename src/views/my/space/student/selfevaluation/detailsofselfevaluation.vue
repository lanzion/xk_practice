<template>
  <div class="detailsofselfevaluation">
    <div class="detailsofstudentworks">
      <div class="atdetails">
        <div class="conent_x">
          <span>课程名称</span>
          <span>{{list.name}}</span>
        ...
        <div class="sdetails" v-show="istrue">
          ...
          <div class="conent_x">
            <span>课程类型</span>
            <span>{{list.courseTypeParentName}}>{{list.courseTypeName}}</span>
          </div>
          <div class="conent_x">
            <span>学习目标</span>
            <span>{{list.target}}</span>
          ...
          <div class="download">
            ...
            <ul>
              <li v-for="(a,index) in list.courseResources" :key="index">
                <span>附件{{index+1}}</span>
                <span>{{a.resourceName}}</span>
                <span>
                  <a :href="a.resourceId">下载</a>
                </span>
              </li>
            </ul>
          </div>

          <div v-for="(g,index) in list.courseClassServers" :key="index">
            ...
            <div class="conent_x">
              <span>基地地址</span>
              <span>{{g.baseAddress}}</span>
            </div>
            <div class="conent_x">
              <span>参与服务</span>
              <span>{{g.serverTitle}}</span>
            </div>
            <div class :style="{'paddingLeft':'40px','margin':'20px 0','overflow':'hidden'}">
              <span
                :style="{'display':'inline-block','fontSize':'16px','color':'#666','vertical-align':'top','margin-right':'20px'}"
        ...
        <div :style="{'width':'100%','height':'390px'}">
          <div class="bigimg">
            <img ref="imgsrc" :src="pimg.resourceId" alt />
          </div>
          <div class="smallimg">
            <ul>
              <li
        ...
        <div :style="{padding:'40px'}">
          ...
          <div class="scoretime" v-for="(g,index) in entity.evalItems" :key="index">
            <div
              class="m1"
        ...
        <div :style="{'padding':'20px 40px'}" v-if="!seetable">
          <textarea
            :style="{'width':'100%'}"
            v-model="remark"
            name
        ...
        <div :style="{'textAlign':'center'}" v-if="!seetable">
    ...
    methods: {
        async baocun() {
            ...
            if (type === 3) {
                ...
                console.log(data)
                let res = await selfevaluationsubmission(data)
                // console.log(res);
                if (res.data.code === 201) {
                    ...
                    this.$router.push({ path: '/space/selfevaluation/ungradedstudents' })
                }
            }
            if (type === 1) {
                let data = {
                ...
                let res = await evaluatingotherssubmit(data)
                // console.log(res);
                if (res.data.code === 201) {
                    this.$message({
                        message: '评语不能为空或分数不能为空',
                    ...
                    this.$router.push({ path: '/evaluatingothers/evaluateotherswithoutscoring' })
                }
            }
        },
        async getlist() {
            let busId = localStorage.getItem('busId')
            let iid = this.$route.query.busId
            let type = this.$route.query.type
            this.seetable = this.$route.query.seetable
            let id = ''
            if (iid) {
                id = iid
            } else {
            ...
            if (type == 1) {
                // 评价他人 未评详情
                res = await detailsofnotevaluatingothers({ busId: id })
            }
            if (type == 2) {
                // 评价他人 已评详情
                res = await detailsofevaluatingothers({ busId: id })
            }
            if (type == 3) {
                // 未评价详情
                res = await getdetailsofnoselfevaluation({ busId: id })
            }
            if (type == 4) {
                // 已评价详情
                res = await getdetailsofselfevaluation({ busId: id })
            }
            this.datas = res.data.entity
            this.entity = res.data.entity.eval || {}
            this.list = this.datas.courseInfo
            this.remark = res.data.entity.eval.remark || ''
            this.totle = res.data.entity.eval.totalScore
            this.busName = res.data.entity.workName
            this.evaluationTemplateId = res.data.entity.eval.evaluationTemplateId
            if (res.data.entity.picResources) {
                this.pimg = res.data.entity.picResources[0] || ''
            }
            if (res.data.entity.accResources) {
                this.playerOptions.sources[0].src =
          res.data.entity.accResources[0].resourceId || ''
            }
            // if (res.data.entity.eval.evaluationTemplateId) {
            // } else {
