    ...
    <div class="g-search--wrap clearfix" style="maxWidth:1200px;min-width: 700px;">
      ...
      >
        ...
        <el-row>
          <el-col :span="15">
            <el-form-item label="所属基地" prop="baseinfoName">
              <el-input v-model="ruleForm.baseinfoName" disabled></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item label="最大参与" prop="maxMannum" class="half">
          <el-input v-model="ruleForm.maxMannum"></el-input>
        </el-form-item>
         <el-form-item label="班级限制" prop="maxClassNum" class="half">
          <el-input v-model="ruleForm.maxClassNum"></el-input>
        </el-form-item>
        <div>
          <el-form-item label="基地时间">
                <span style="marginRight:15px;color:#606266;">基地上班时间:{{detail.startTime}}</span>
                <span style="marginRight:15px;color:#606266;">基地午休时间:{{detail.startLunchHour}}-{{detail.endLunchHour}}</span>
                <span style="marginRight:15px;color:#606266;">基地下班时间:{{detail.endTime}}</span>
            </el-form-item>
        </div>
        <el-form-item label="活动时长" prop="maxTime" class="half">
           <el-select v-model="ruleForm.maxTime" placeholder="请选择活动时长" @change="checkNum" clearable>
            <el-option
        ...
        <el-form-item label="日办次数" prop="maxFrequency" class="half" >
          <el-input v-model="ruleForm.maxFrequency" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="服务时间段" v-show="timeArr.length>0">
             <div class="school-time-box">
                <span class="school-time-btn" v-for="(item,index) in timeArr"  :key="index" >{{`${item.start}~${item.end}`}}</span>
            </div>
        </el-form-item>
          
        <el-form-item label="负责人" prop="personInCharge" class="half">
          <el-input v-model="ruleForm.personInCharge"></el-input>
        </el-form-item>
        <el-form-item label="联系方式" prop="personInChargePhone" class="half">
          <el-input v-model="ruleForm.personInChargePhone"></el-input>
        </el-form-item>
        <el-form-item label="服务位置" prop="address" class="half">
          <el-input v-model="ruleForm.address"></el-input>
        </el-form-item>
        <el-form-item label="服务说明" prop="expound">
          <el-input type="textarea" v-model="ruleForm.expound"></el-input>
        </el-form-item>
        ...
        <el-form-item label="服务图片" prop="cover">
            ...
            @remove="remove"
            >
            <p class="upload-list-tips">仅支持JPG、GIF、PNG、JPEG格式，文件小于 {{ imgStandardFileSize }} M。500*500像素比例</p>
          </ali-upload>
        </el-form-item>
      </el-form>
      <div :style="{'width':'100%','textAlign':'center','marginTop':'20px'}">
        <el-button type="primary" plain @click="goback">取消</el-button>
        <el-button type="primary" @click="submit" v-if="isSheng">提交审核</el-button>
      </div>
    </div>
  </div>
</template>
...
import { baseInfoAdd, baseinfoDetail, baseinfoEdit,adminBaseInfoDetail } from "@/api/newApi";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        cover:[{
             required: true, message: '请选择服务图片', trigger: ['change','blur']
        }],
        title: [
          {
            required: true,
  ...
  methods: {
    ...
    submit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          formList.personInChargePhone = Number(
            formList.personInChargePhone
            );
    ...
    checkNum(){
      if(!this.detail.startTime||!this.detail.endTime||!this.detail.startLunchHour||!this.detail.endLunchHour){
           this.$message({
                  message: '基地作息时间有误',
                  type: "warning"
                });
          ...
          let startTime = parseInt(this.detail.startTime.slice(0,2)),
              ...
              arr =[],
              arr1=[];
         if(startTimeHalf>0){//判断上班时间是否大于当前整数时
           startTime = startTime+1;
         }
         if(endLunchHourHalf>0){//判断上班时间是否大于当前整数时
        ...
        //  console.log(num2)
           if(num1>=1){
             ...
             for(let i=0;i<num1;i++){
               let start =startTime+i*this.ruleForm.maxTime>9?startTime+i*this.ruleForm.maxTime:`0${startTime+i*this.ruleForm.maxTime}`,
                   end =startTime+(i+1)*this.ruleForm.maxTime>9?startTime+(i+1)*this.ruleForm.maxTime:`0${startTime+(i+1)*this.ruleForm.maxTime}`;
               arr.push({
                 start:start+':00',
                 end:end+':00'
               })
            ...
            if(num2>=1){
             ...
             for(let i=0;i<num2;i++){
               let start =endLunchHour+i*this.ruleForm.maxTime>9?endLunchHour+i*this.ruleForm.maxTime:`0${endLunchHour+i*this.ruleForm.maxTime}`,
                   end =endLunchHour+(i+1)*this.ruleForm.maxTime>9?endLunchHour+(i+1)*this.ruleForm.maxTime:`0${endLunchHour+(i+1)*this.ruleForm.maxTime}`;
               arr.push({
                 start:start+':00',
                 end:end+':00'
               })
        ...
        baseinfoDetail({ id: id })
          .then(res => {
            ...
            formData.cover = [formData.cover]
              ...
              if(this.ruleForm.cover) this.cover = [
                 {
                    name: "u200.jpg",
                    size: 37110,
                    status: "success",
