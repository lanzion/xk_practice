...
import {getActivityTypeParent,templateEdit,getGradeList,templateDetail} from '@/api/resetApi'
        ...
        getDetailData() {
            ...
            if (id) {
                ...
                templateDetail({ id }).then(res => {
                    ...
                    if (code === 200 && datas) {
                          this.form.title = datas.templateName;
                          this.form.group = datas.templateType;
                          this.form.classId = datas.templateGrade;
                          this.form.remark = datas.remark;
                             this.form.id=datas.id;
                        
                          this.form.values=[datas.activityField,datas.activityType];
                         let arr =  this.options.filter(o=>o.value==datas.templateSection)[0].key;
                     
                          this.form.schoolType = arr;
                        this.getGradeAll();
                 ...
                 if(len==1&&index==0){//判断是否剩下最后一项
                      this.$message({
                        ...
                        if(len==1){//判断是否只有一项
                            ...
                            this.tree[index].defContentItems = arr;
                        }
                        this.getGradeAll();
