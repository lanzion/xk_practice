<template>
  <div class="content">
    <div class="school-detail-main-list">
      <span class="school-detail-list-head">活动标题</span>
      <div class="school-detail-list-redource">{{data.title}}</div>
    </div>
    <div class="school-detail-main-list">
      <span class="school-detail-list-head">活动时间</span>
      <div class="school-detail-list-redource">{{data.startDate}} 至 {{data.endDate}}</div>
    </div>
    <div class="school-detail-main-list">
      <span class="school-detail-list-head">活动主题</span>
      <div class="school-detail-list-title">{{data.activityTheme}}</div>
    </div>
    <div class="school-detail-main-list">
      <span class="school-detail-list-head">活动指标</span>
      <div class="school-detail-list-title">{{data.activityTarget}}</div>
    </div>
    <div class="school-detail-list">
      <div class="school-detail-item">
        <span class="school-detail-list-head">封面图片</span>
        <div class="school-detail-list-title">
          <el-image :src="getFileUrl(data.cover)" class="school-detail-img">
      ...
      <div class="school-detail-item">
        <span class="school-detail-list-head">详情海报</span>
        <div class="school-detail-list-title">
          <el-image :src="getFileUrl(data.playbillPage)" class="school-detail-img">
            <div slot="placeholder" class="image-slot">
              加载中
              <span class="dot">...</span>
            </div>
    ...
    <div class="school-detail-main-list">
      <span class="school-detail-list-head">活动资源</span>
      <div class="school-detail-list-redource">
        <div
          class="school-detail-redource-list"
          v-for="(item,index) in data.resourceVoList"
          :key="index"
        >
          <span class="school-detail-name">附件{{index+1}}</span>
          <span class="school-detail-name-title">{{item.name}}</span>
          <!-- <a :href="item.resourceId" :download="item.name" class="school-detail-download-btn">下载</a> -->
          <a
    ...
    <div class="school-detail-main-list">
      <span class="school-detail-list-head">活动通知</span>
      <div class="school-detail-list-redource">
        <div v-if="data.schActVoList&&data.schActVoList.length>0">
          <span v-for="(item,index) in data.schActVoList" :key="index" style="marginRight:15px;">
            {{item.schoolName}}(
            <template v-if="item.status=='A'">已查看</template>
            <template v-else>未查看</template>)
          </span>
        </div>
        <div v-if="data.schActVoList&&data.schActVoList.length==0">暂无学校</div>
      </div>
    </div>
    <div class="school-detail-main-list">
