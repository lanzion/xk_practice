    ...
    <div class="openactivityinfo">
        ...
        <div class="clearfix">
            ...
            <el-col>
                ...
                <el-col :span="20">
                    <el-image
                        :src="getFileUrl(actData.cover)"
                        class="school-detail-img"
                        @click="prewImage"
                    >
            ...
            <el-col>
                ...
                <el-col
                    :span="20"
                >{{actData.enrollStartDate}} 至 {{mymoment(actData.enrollEndDate).format('YYYY-MM-DD')==mymoment(actData.enrollStartDate).format('YYYY-MM-DD')?mymoment(actData.enrollEndDate).format('hh:mm:ss'):actData.enrollEndDate}}</el-col>
            </el-col>
            <el-col>
                ...
                <el-col :span="20" class="activity_box">
                    <el-col :span="6">
                        <p
                            ...
                            :key="item.id"
                        >{{item.activityStartDate}} 至 {{mymoment(item.activityEndDate).format('YYYY-MM-DD')==mymoment(item.activityStartDate).format('YYYY-MM-DD')?mymoment(item.activityEndDate).format('hh:mm:ss'):item.activityEndDate}}</p>
                    </el-col>
                    <el-col
                        :span="4"
        ...
        <div class="btn_list">
            ...
            <el-button
                type="primary"
                @click="submi"
                :disabled="actData.status!=1"
            >{{actData.status|submiTxt}}</el-button>
        </div>
                        ...
                        if (isPass) {
                            this.$confirm('恭喜，活动确认成功！', '', {
                                ...
                                .then(() => {
                                    this.$router.push({
                                        path:
                                            '/practicalManage/openActivityList',
                    ...
                    if (data.code === 200) {
                        ...
                        this.activityTypeParent.forEach(v => {
                            if (v.code === this.actData.typeParent) {
                                this.actData['typeParentName'] = v.name
                            }
                        })
                    } else {
                        this.$message({
