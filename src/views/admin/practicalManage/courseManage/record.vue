<template>
    <div class="content">
        ...
        <div class="school-detail-main-list">
                ...
                <div class="school-detail-list-redource">
                     {{datas.courseTypeParentName}}/{{datas.courseTypeName}}
                </div>
        </div>
        <div class="school-detail-main-list">
        ...
        <div class="course_scheduling_box">
                ...
                <div class="course_scheduling_main">
                        <div class="course_scheduling_item"  @click="openDialog(item)" v-for="item in datas.baseinfoArray" :key="item.baseInfoId">
                            <div class="course_scheduling_img_box">
                                <el-image :src="item.baseInfoCover" class="course_scheduling_img" fit="cover">
                ...
                <div class="course-dialog-main">
                    <p class="course-dialog-head">基地服务列表</p>
                     <div class="school-table-box">
                        <el-table  :data="list"  stripe align="center"  border :header-cell-style="{'background-color':'#eee', 'color':'#666'}" width="100%" max-height="300">
                                ...
                                <template slot-scope="scope">
                                    <div class="school-table-list-box">
                                        <div class="school-table-list" :class="{activeIndex:activeIndex!=scope.$index}">
                                                服务时长：{{scope.row.maxTime}} 小时<br/>
                                                日办次数：{{scope.row.maxFrequency}}次<br/>
                                                服务人群：{{scope.row.scope}} <br/>
                                                服务人数：每次{{scope.row.minMannum||1}}-{{scope.row.maxMannum}}人<br/>
                                                服务描述：{{scope.row.expound}}
                                        </div>
                                        <span class="school-table-list-btn" v-show="activeIndex!=scope.$index" @click="closeEs(scope.$index,'open')">展开</span>
                                        <span class="school-table-list-btn" :class="{activeIndex:activeIndex==scope.$index}" v-show="activeIndex==scope.$index"  @click="closeEs(scope.$index,'close')">收起</span>
                                    </div>
                            ...
                            <el-table-column label="操作" align="center" width="70">
                                 <template slot-scope="scope">
                                       <el-button class="g-table--link" type="text"   @click="changeItem(scope.row,scope.$index)" v-if="scope.row.id==form.projectId">
                                            取消
                                        </el-button>
                                         <el-button class="g-table--link" type="text"   @click="changeItem(scope.row,scope.$index)" v-else>
                                             选择
                                        </el-button>
                                  </template>
                            </el-table-column>
                            ...
                            <div class="school-detail-list-redource" style="lineHeight:32px;">
                                  <el-form-item>
                                            <el-date-picker
                                                ...
                                                clearable
                                                type="date"
                                               :picker-options="pickerOptions0" style="width:140px;" @change="getDate">
                                            </el-date-picker >
                                            <span>{{week}}</span>
                                            <el-button  style="marginLeft:30px;" type="primary" @click="openDetail" v-show="week!=''">当日日程</el-button>
                                    </el-form-item>
                    ...
                    <div class="school-detail-main-list" v-show="time.length!=0">
                            ...
                            <div class="school-detail-list-redource">
                                   <div class="school-time-box">
                                        <span class="school-time-btn" v-for="(item,index) in time" :class="{'oy-active':timeIndex===item.timeCode}" :key="index" @click="changeTime(item,index)">{{item.timeCode}}</span>
                                   </div>
                            </div>
                    </div>
                     <div class="school-detail-main-list">
...
import { selectCourse,getClassAndNum,baseinfoProject,comfirCourse,baseprojectCourseTime} from '@/api/resetApi'
export default {
    data() {
        return {
            pickerOptions0: { 
    ...
    methods: {
        submit(){
           ...
           if(this.timeIndex===''){
             ...
             formList.timecodetxt = this.timeIndex;
               this.isLoading = true
                comfirCourse(formList).then(res => {
        ...
        changeItem(row,index){//选择服务项
            if(this.changIndex ===index){
                ...
                baseprojectCourseTime({ projectId:row.id }).then(res => {
         ...
         changeTime(item,index){//选择排课时间
            if(this.form.projectId===''){
                  ...
                  this.timeIndex===item.timeCode?this.timeIndex='':this.timeIndex=item.timeCode;
            }
        },
       ...
       changeValue(){//选择年级
        ...
        openDetail(){
             this.$router.push({path:'/practicalManage/baseAuthortySchedule/baseAuthortyScheduleDetail',query:{
        ...
        openDialog(row){
            ...
            baseinfoProject({baseinfoId:row.baseInfoId,auditStatus:"A"}).then(res => {
