   ...
   <div class="school-list-wrap">
        ...
        <div class="g-search--wrap clearfix">
            <el-form :inline="true"  class="fl elFrom" @submit.native.prevent>
                 ...
                 <el-form-item label="活动标题">
                    <el-input v-model="form.activityTitle" placeholder="请输入活动标题" @keyup.native.enter="resetPage"></el-input>
                </el-form-item>
        ...
        <el-table  ref="table" :data="listData" stripe align="center" v-loading="isLoading" border :header-cell-style="{'background-color':'#eee', 'color':'#666'}">
            ...
            <el-table-column label="操作" align="center" >
                <template slot-scope="scope">
                      <el-button class="g-table--link" type="text" v-if="scope.row.commentStatus=='A'"  @click="goDetail(scope.row,'detail')">
                          点评详情
                      </el-button>
                       <el-button class="g-table--link" @click="goDetail(scope.row,'add')" type="text" v-else>
                           活动点评
                      </el-button>
                </template>
...
export default {
    ...
    watch: {
     ...
     methods: {
        goDetail(row,key){
            if(key=='add'){
                 this.$router.push({path:`/practicalManage/practicalComment/${key}`,query:{id:row.id,schoolName:row.schoolName,activityId:row.activityId,schoolId:row.schoolId}})
            }else{
                 this.$router.push({path:`/practicalManage/practicalComment/${key}`,query:{id:row.id}})
            }
