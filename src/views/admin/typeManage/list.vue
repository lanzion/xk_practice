             ...
             <el-table-column type="expand">
                <template slot-scope="props">
                          <div class="model-table-box">
                            ...
                            <div class="model-table-tr" v-for="(item,index) in props.row.dicDetailList" :key="index">
                                ...
                                <div class="model-table-td">
                                  <el-switch
                                        ...
                                        off-text="停用"
                                        off-value="0"
                                        v-model="item.status" @change="changeSwitch1(item)">
                                     </el-switch>
                                </div>
                                <div class="model-table-td" >
                                       {{item.remark}}
                                ...
                                <div class="model-table-td" >
                                      <el-button class="g-table--link" type="text"   @click="changeItem(item)">
                                            编辑
                                        </el-button>
                                </div>
                            </div>
                            <div class="model-table-tr" v-if="props.row.dicDetailList&&props.row.dicDetailList.length==0||!props.row.dicDetailList" >
                                <div class="model-table-td" style="width:100%;">
                                    暂无数据
                                </div>
...
import {activityType,activityTypUpdate,activityTypeDetailUpdate} from '@/api/resetApi'
...
export default {
    ...
    methods: {
         changeSwitch (data={}) {
            ...
            activityTypUpdate(formData).then(res => {
         ...
         changeSwitch1 (data={}) {
            ...
            const formData ={
             ...
             data.status?formData.status='A':formData.status='B';
            activityTypeDetailUpdate(formData).then(res => {
                if (res.data.code === 200) {
