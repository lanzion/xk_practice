    ...
    <div class="g-search--wrap clearfix">
      ...
      <div class="fr">
        <section class="g-table--head clearfix fr">
          <head-operate class="fr" :items="headBtnGroup" v-bind="{ pass:{ callback: pass },}" />
        </section>
      </div>
    </div>
    <el-table
      ...
      <el-table-column label="操作" align="center" fixed="right" :width="operateWidth">
        <template slot-scope="scope">
          <list-operate
            ...
            :data="scope.row"
            :index="scope.$index"
            v-bind="{  del: {callback:del},edit:{query: { id: 'id'}} }"
          />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getDatas() {
      ...
      const res = await activityResultDraftUnPassList(formList, this.pages);
    ...
    pass() {
      ...
      resultAuditBatchCommit({ activityResultIds: activityResultIds }).then(
        res => {
          try {
            let datas = res.data;
            if (datas.code == 200) {
