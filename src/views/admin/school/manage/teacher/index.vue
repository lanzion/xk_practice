  ...
  <div class="school-list-wrap">
    ...
    >
      ...
      <el-table-column prop="teach" label="任教学科">
        ...
        <template
          slot-scope="scope"
        >{{ scope.row.teach | translate(technicalClassification, { con: ' / ' }) }}</template>
      </el-table-column>
      <el-table-column prop="provience" label="所属区域">
        <template
          slot-scope="scope"
        >{{ scope.row.provienceName || '' }}{{ scope.row.cityName || '' }}{{ scope.row.areaName || '' }}</template>
      </el-table-column>
      ...
      <el-table-column prop="lockStatus" label="账号状态" align="center" width="120">
        ...
        <template slot-scope="scope">
          <span
            :class="['lockStatus', `__status-${scope.row.lockStatus}`, { 0: 'icon-unlock', 1: 'icon-lock' }[scope.row.lockStatus]]"
          >{{ scope.row.lockStatus | translate(lockStatus) }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="auditStatus" label="审核状态" align="center" width="110">
        ...
        <template slot-scope="scope">
          <span
            :class="['auditStatus', `__status-${scope.row.auditStatus}`]"
          >{{ scope.row.auditStatus | translate(auditStatus) }}</span>
        </template>
      </el-table-column>
...
export default {
    ...
    computed: {
        ...mapState('dict', {
            technicalClassification: state =>
                ((state.technicalClassification || {}).dicList || []).map(o => {
                    return {
                        code: o.code,
                        id: o.code,
                        name: o.name
